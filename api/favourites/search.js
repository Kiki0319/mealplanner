...
module.exports = async (req, res) => {
  ...
  // Trim env vars to avoid accidental whitespace from UI or .env files
  const RECIPE_API_ID = (process.env.RECIPE_API_ID || '').trim();
  const RECIPE_API_KEY = (process.env.RECIPE_API_KEY || '').trim();
  const RECIPE_API_BASE_URL = (process.env.RECIPE_API_BASE_URL || 'https://api.edamam.com/api/recipes/v2').trim();
  ...
  try {
    ...
    if (err.response) {
      ...
      // Forward Edamam response to client for clearer error messages (helps debugging)
